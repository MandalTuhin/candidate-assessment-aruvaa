          ...
          >
            <div class="p-3 sm:p-4">
              <div class="flex items-start justify-between mb-2">
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-1">
                    <span 
                      ...
                      :class="{
                        'bg-green-100 text-green-700': question.status === 'correct',
                        'bg-red-100 text-red-700': question.status === 'incorrect',
                        'bg-yellow-100 text-yellow-700': question.status === 'skipped'
                      }"
                    >
                ...
                <div class="ml-3">
                  <!-- Correct Icon -->
                  <svg v-if="question.status === 'correct'" class="w-6 h-6 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
                  </svg>
                  <!-- Incorrect Icon -->
                  <svg v-else-if="question.status === 'incorrect'" class="w-6 h-6 text-red-600" fill="currentColor" viewBox="0 0 20 20">
              ...
              <div class="mt-3 space-y-2">
                <!-- Skipped Question -->
                <div v-if="question.status === 'skipped'" class="p-2.5 sm:p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                ...
                <div 
                  ...
                  :class="{
                    'bg-green-50 border border-green-200': question.status === 'correct',
                    'bg-red-50 border border-red-200': question.status === 'incorrect'
                  }"
                >
                  <span 
                    class="text-xs sm:text-sm font-medium"
                    :class="{
                      'text-green-700': question.status === 'correct',
                      'text-red-700': question.status === 'incorrect'
                    }"
                  >
                    Your Answer:
                  </span>
                  <span 
                    ...
                    :class="{
                      'text-green-800': question.status === 'correct',
                      'text-red-800': question.status === 'incorrect'
                    }"
                  >
                ...
                <div v-if="question.status !== 'correct'" class="p-2.5 sm:p-3 bg-green-50 border border-green-200 rounded-md">
